<div class="space-y-8">
    ...
    <div class="bg-white dark:bg-zinc-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-zinc-700 mb-8 transition-colors duration-300 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div class="text-center">
            ...
            <p class="text-3xl font-extrabold text-red-600 dark:text-lime-400 mt-1"><span>{{ number_format($this->totalAchieved ?? 0, 1) }}</span><span class="text-lg font-medium">g</span></p>
        </div>
        <div class="text-center md:text-right">
            ...
            @endphp
            @if($remaining <= 0)
                <p class="text-3xl font-extrabold text-emerald-500 mt-1"><span>Tercapai!</span></p>
            @else
                <p class="text-3xl font-extrabold text-orange-500 dark:text-orange-400 mt-1"><span>{{ number_format($remaining, 1) }}</span><span class="text-lg font-medium">g</span></p>
            @endif
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="md:col-span-1 bg-white dark:bg-zinc-800 p-6 rounded-2xl border border-gray-100 dark:border-zinc-700 shadow-sm h-fit space-y-5 transition-colors duration-300">
            <div class="flex justify-between items-center border-b border-gray-100 dark:border-zinc-700 pb-3">
                <h3 class="font-bold text-red-600 dark:text-lime-400 transition-colors duration-300">➕ Log Protein</h3>
                <span class="text-sm font-bold bg-red-100 dark:bg-lime-900/30 text-red-700 dark:text-lime-400 px-3 py-1.5 rounded-lg transition-colors duration-300">{{ $this->calculatedProtein ?? 0 }}g</span>
            </div>

            <form wire:submit.prevent="saveLog" class="space-y-4">
                <div>
                    <input type="date" wire:model.live="tanggal" 
                           class="w-full border border-gray-200 dark:border-zinc-600 bg-gray-50 dark:bg-zinc-900/50 text-slate-800 dark:text-white p-3 rounded-xl focus:ring-2 focus:ring-red-500 dark:focus:ring-lime-400 outline-none transition-colors duration-300">
                </div>
                ...
                >
                    ...
                    <button type="button" @click="open = !open" 
                            class="w-full flex justify-between items-center border border-gray-200 dark:border-zinc-600 bg-gray-50 dark:bg-zinc-900/50 text-slate-800 dark:text-white p-3 rounded-xl focus:ring-2 focus:ring-red-500 dark:focus:ring-lime-400 outline-none transition-all duration-300 text-left shadow-sm hover:border-red-300 dark:hover:border-lime-500/50">
                        <span x-text="selectedLabel" class="truncate font-medium text-sm"></span>
                        <svg class="w-5 h-5 text-gray-400 transition-transform duration-300" :class="{'rotate-180 text-red-500 dark:text-lime-400': open}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    ...
                    <div x-show="open" 
                        ...
                        <div class="p-2 border-b border-gray-100 dark:border-zinc-700">
                            <input type="text" x-model="search" placeholder="Cari makanan..." class="w-full p-2 text-sm bg-gray-50 dark:bg-zinc-900/50 border border-gray-200 dark:border-zinc-600 rounded-lg focus:outline-none focus:ring-1 focus:ring-red-500 dark:focus:ring-lime-400 text-slate-800 dark:text-white placeholder-gray-400">
                        </div>
                        ...
                        <div class="overflow-y-auto hide-scrollbar">
                            <template x-for="source in filteredSources" :key="source.name">
                                <div @click="selected = source.name; open = false; search = '';" 
                                     ...
                                     :class="{'bg-red-50 text-red-600 dark:bg-zinc-700/50 dark:text-lime-400': selected === source.name, 'text-gray-700 dark:text-zinc-300': selected !== source.name}">
                                    <span x-text="source.label"></span>
                                    <svg x-show="selected === source.name" class="w-4 h-4 text-red-600 dark:text-lime-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                                </div>
                            </template>
                ...
                <div class="relative">
                    <input type="number" step="0.5" wire:model.live="berat" 
                           class="w-full border border-gray-200 dark:border-zinc-600 bg-gray-50 dark:bg-zinc-900/50 text-slate-800 dark:text-white p-3 pr-16 rounded-xl focus:ring-2 focus:ring-red-500 dark:focus:ring-lime-400 outline-none transition-colors duration-300"
                ...
                <button type="button" wire:click="saveLog" 
                        class="w-full bg-red-600 dark:bg-lime-400 text-white dark:text-zinc-900 font-bold p-3 rounded-xl hover:bg-red-700 dark:hover:bg-lime-500 transition-colors duration-300 shadow-md flex justify-center items-center gap-2">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-zinc-700">
                            @forelse($this->dailyLogs ?? [] as $log)
                                <tr class="hover:bg-gray-50 dark:hover:bg-zinc-700/30 transition-colors" wire:key="log-item-{{ $log->id }}">
                                    ...
                                    <td class="p-4 font-bold text-red-600 dark:text-lime-400">
                                    ...
                                    <td class="p-4 text-center">
                                        <button wire:click="deleteLog({{ $log->id }})" 
                                                ...
                                                class="px-3 py-1.5 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 rounded-lg font-bold text-xs transition-all">
                                            Hapus
                                        </button>
                                    </td>
                                </tr>
