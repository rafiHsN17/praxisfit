            ...
            <div class="flex justify-between h-16 items-center">
                ...
                <div class="hidden md:flex items-center gap-2 text-sm tracking-tight">
                    <a href="{{ route('admin.dashboard') }}" wire:navigate 
                       class="px-4 py-2 rounded-xl transition-colors {{ request()->routeIs('admin.dashboard') ? 'text-red-600 dark:text-lime-400 font-bold bg-slate-100 dark:bg-zinc-800 shadow-sm' : 'text-zinc-600 dark:text-zinc-300 hover:text-red-600 dark:hover:text-lime-400 font-semibold' }}">
                        Dashboard
                    </a>
                    <a href="{{ route('admin.exercises') }}" wire:navigate 
                       class="px-4 py-2 rounded-xl transition-colors {{ request()->routeIs('admin.exercises') ? 'text-red-600 dark:text-lime-400 font-bold bg-slate-100 dark:bg-zinc-800 shadow-sm' : 'text-zinc-600 dark:text-zinc-300 hover:text-red-600 dark:hover:text-lime-400 font-semibold' }}">
                        Kelola Gerakan
                    </a>
                    <a href="{{ route('admin.protein-sources') }}" wire:navigate 
                       class="px-4 py-2 rounded-xl transition-colors {{ request()->routeIs('admin.protein-sources') ? 'text-red-600 dark:text-lime-400 font-bold bg-slate-100 dark:bg-zinc-800 shadow-sm' : 'text-zinc-600 dark:text-zinc-300 hover:text-red-600 dark:hover:text-lime-400 font-semibold' }}">
                        Kelola Kalkulator Protein
                    </a>
                </div>
                ...
                <div class="flex items-center gap-3">
                    @auth('admin')
                        <div class="hidden sm:flex items-center gap-2">
                            ...
                            <a href="{{ url('/') }}" class="text-xs font-semibold px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors">
                                Keluar ke Web User
                            </a>
                            <form method="POST" action="{{ route('admin.logout') }}" class="inline m-0 p-0 ml-1">
                                @csrf
