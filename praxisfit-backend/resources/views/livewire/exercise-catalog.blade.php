<div class="space-y-8 sm:space-y-12">
    ...
    <div class="space-y-4">
        ...
        <p class="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed max-w-2xl">
            Temukan panduan latihan yang tepat untukmu. Cari berdasarkan nama gerakan atau target otot.
        </p>
    </div>
        ...
        @forelse($exercises as $exercise)
            <div wire:click="openExerciseDetail({{ $exercise->id }})"
                 ...
                 x-data="{ 
                     ...
                     frame: 0, 
                     src0: '{{ $exercise->icon }}',
                     src1: '{{ str_replace('0.jpg', '1.jpg', $exercise->icon) }}'
                 }"
                 @mouseenter="timer = setInterval(() => { frame = frame === 0 ? 1 : 0 }, 600)"
                ...
                <!-- Media (Edge-to-Edge with Play-on-Hover Illusion) -->
                <div class="relative overflow-hidden border-b border-zinc-200 dark:border-zinc-800 h-52">
                    @if($exercise->is_custom && $exercise->embed_url)
                        <iframe class="w-full aspect-video object-cover rounded-t-xl" src="{{ $exercise->embed_url }}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                    @else
                        <!-- The original ExerciseDB GIF -->
                        <img :src="frame === 0 ? src0 : src1" src="{{ $exercise->icon }}" class="w-full h-full object-cover object-center transition-transform duration-700 group-hover:scale-110 bg-white dark:bg-zinc-800" alt="{{ $exercise->name }}">
                        <!-- The Dark Glass Overlay for GIF only -->
    ...
    @if($selectedExercise)
        <template x-teleport="body">
            <div x-data="{ 
                 ...
                 x-show="open" 
                 x-init="
                     modalTimer = setInterval(() => { frame = frame === 0 ? 1 : 0 }, 600);
                ...
                <div x-show="open"
                    ...
                    <div class="p-4 sm:p-6 overflow-y-auto hide-scrollbar space-y-5 sm:space-y-6">
                        ...
                        <div class="w-full aspect-video bg-white dark:bg-zinc-950 rounded-lg flex items-center justify-center relative border border-zinc-200 dark:border-zinc-800 overflow-hidden">
                            @if($selectedExercise->is_custom && $selectedExercise->embed_url)
                                <iframe class="w-full h-full object-cover" src="{{ $selectedExercise->embed_url }}?autoplay=1" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                            @elseif($selectedExercise->icon)
                                <img :src="frame === 0 ? '{{ $selectedExercise->icon }}' : '{{ str_replace('0.jpg', '1.jpg', $selectedExercise->icon) }}'" src="{{ $selectedExercise->icon }}" class="w-full h-full object-contain" alt="{{ $selectedExercise->name }}">
                            @else
                    ...
                    <div class="space-y-3 pt-4 border-t border-zinc-200 dark:border-zinc-800">
                        ...
                        @php
                            // Kamus Istilah Binaraga/Medis Khusus
                            $terms = [
